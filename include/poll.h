...
#include <sys/epoll.h>
/**
 * struct mrepoll_server_t
 * 	int efd 		: epoll file decription.
 * 	int max_event	: max event focus on.
...
#define CALLBACK(x) void (*x) (int listen_sock ,struct epoll_event *ev ,mrepoll_server_t * ms)
#define CALLBACKCLOSE(x) void (*x)  (mrepoll_server_t * ms,int fd);
